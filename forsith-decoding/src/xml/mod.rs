...
use forsith_proc::{Deref, IsVariant};
use forsith_shared::{
    bail, ensure, errmsg,
    error::{Error, Result, ResultContext},
...
impl Encoding {
    ...
    fn from_str(s: &str) -> Result<Self> {
        if s.eq_ignore_ascii_case("utf-8") {
            return Ok(Self::Utf8);
        } else if s.eq_ignore_ascii_case("utf-16") | s.eq_ignore_ascii_case("utf-16le") {
            return Ok(Self::Utf16LE);
        } else if s.eq_ignore_ascii_case("utf-16be") {
            return Ok(Self::Utf16BE);
        }
...
impl XmlDocument {
    #[must_use]
    pub const fn prolog(&self) -> &Prolog {
        &self.prolog
    }
    #[must_use]
    pub const fn tree(&self) -> &XmlTree {
        &self.tree
    }
    ...
    pub fn parse_with_interner(
        ...
        let prolog = data.prolog(interner).with_context(|| {
            format!(
                "Failed to parse prolog, error was likely located around {}",
                data.current_source_position()
            )
        })?;

        ensure!(
            prolog.encoding == encoding,
            "Encoding mismatch: prolog specifies {:?}, but detected {encoding:?}",
            prolog.encoding
        );

        let tree = XmlTree::parse(&mut data, interner).with_context(|| {
            format!(
                "Failed to parse elements, error was likely located around {}",
                data.current_source_position()
            )
        })?;
